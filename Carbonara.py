...
def safe_download_button(df, label, filename):
    ...
    st.download_button(label, csv_bytes, file_name=filename, mime='text/csv/xlsx')

#data input (csv)
st.sidebar.header("1) Data input (CSV)")
uploaded = st.sidebar.file_uploader("Upload CSV file (or skip to use default CSV)", type=["csv", "txt", "xlsx"])
if uploaded:
...
#detect columns & parse dates (Tiara made changes here)
...
with st.sidebar.expander("Manual column overrides (if detection is wrong)"):
    ...
    def get_index(col_name):
        #return index 
        return 0 if col_name is None or col_name not in df.columns else col_options.index(col_name)

    date_col = st.selectbox("Date column", options=col_options, index=get_index(date_col))
    state_col = st.selectbox("State/Province column", options=col_options, index=get_index(state_col))
    sector_col = st.selectbox("Sector column", options=col_options, index=get_index(sector_col))
    metric_col = st.selectbox("Metric column (emissions)", options=col_options, index=get_index(metric_col))

#confirm final selections
st.sidebar.markdown("Final Column Selections")
st.sidebar.write({
    "Date": date_col,
    "State": state_col,
    "Sector": sector_col,
...
#sector filter (Tiara made changes until here)
if sector_col:
